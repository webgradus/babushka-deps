...
dep 'eye.installed' do
  met? {
    rvm_run_with_ruby("2.3.1", "gem list eye") {|shell| shell.stdout.include?("eye") }
  }
  meet {
    rvm_run_with_ruby "2.3.1", "gem install eye --no-rdoc --no-ri"
    rvm_run_with_ruby "2.3.1", "gem install eye-http --no-rdoc --no-ri"
    eye_lib_path = rvm_run_with_ruby "2.3.1", "gem which eye"
    #puts eye_lib_path
