dep 'prepare-deploy', :app_name, :git_username, :server_ip, :app_path do
  ...
  app_path ||= ""
  met? {
    Babushka::Renderable.new((app_path / "config/deploy.rb").to_s).from?(dependency.load_path.parent / "development/deploy.rb.erb") && Babushka::Renderable.new((app_path / "config/unicorn.rb").to_s).from?(dependency.load_path.parent / "development/unicorn.rb.erb")
  }
  meet {
