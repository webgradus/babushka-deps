...
dep 'rvm default ruby is set', :default_ruby do
  ...
  meet do
    while current_rubies.empty?
      ruby = Babushka::Prompt.get_value("You need at least one ruby installed. Which one do you want to install?", :default => "2.3.0")
...
dep 'rvm defaults are installed' do
  ...
  define_var :rubies, :default => "ruby-2.3.0", :message => "which rubies do you want to create? (seperate by ,)"
