...
dep 'locomotive.local', :host, :app_name do
  ...
  meet {
    cd "/opt" do
      rvm_run_with_ruby "1.9.3", "rails _3.2.13_ new #{app_name} --skip-active-record --skip-test-unit --skip-javascript --skip-bundle"
...
dep 'locomotive', :host, :app_name, :port do
  ...
  meet {
    as('root') {
      ...
      remote_babushka "webgradus:unicorn-init-script", :app_name => app_name, :app_type => "locomotive", :database => "mongodb"
