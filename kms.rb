...
dep 'kms installed', :app_name, :ruby_version, :postgres_password do
  ...
  requires 'rails installed'.with(ruby_version, "5.0.1") #, 'repo accessible'.with("git@gitlab.com:webgradus/kms.git")
  ...
  meet {
    cd "/opt" do
      rvm_run_with_ruby ruby_version, "rails _5.0.1_ new #{app_name} --skip-test-unit --skip-bundle --database=postgresql"
